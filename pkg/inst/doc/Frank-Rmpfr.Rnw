...
does not directly work with the copula family object, but gets the three
...
mlogL <- function(theta) -sum(dDiagA(U., theta, d=d, psiInv = psiInv.2,
		psiDabs = psiDabs.3, psiInvD1abs = psiInvD1abs.2, log = TRUE))
@
Unfortunately, this improves the situation for large $\theta$ only slightly:
<<llog-theta-plot-3,fig=true, echo=FALSE>>=
plot(nll ~ thet, xlab=expression(theta),
     ylab = expression(- logLik(theta, .)),
     type = "l", col="red2", lwd=2)
abline(v = th0, col="red2", lty="15", lwd=2)
nll3 <- vapply(thet, mlogL, 0.)
lines(nll3 ~ thet, col = "blue2", lwd=3, lty=2)
mlogL(800)
@
where we can see, that this time, the numerical overflow to $\infty$
happens in \code{psiDabs(d*psiInv(u,th), th, log = TRUE)}, as
\code{d*psiInv(u,th) = 5*psiInv(0.999, th=800)}$=$\Sexpr{format(d*psiInv.2(u=0.999,th=800))}
underflows to zero (by necessity).
The only solution here is in allowing to pass the \emph{logarithm}
