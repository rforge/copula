...
%
%% almost as usual
\author{Marius Hofert\thanks{The\ author\ (Willis\ Research\ Fellow)\ thanks\ Willis\ Re\ for\ financial\ support\ while\ this\ work\ was\ completed.} \\ETH Zurich
 \And
...
%% also  *outer power* versions of our families
  As a by-product, algorithms for
...
if(Sys.getenv("USER") == "maechler")# take CRAN's version, not development one:
    library(nacopula, lib="/usr/local/app/R/R_local/library")
@
...
Alongside \textit{elliptical copulas}, i.e., the copulas arising from elliptical
 ...
 \citet{embrechtslindskogmcneil2001}), Archimedean copulas play an important
 role  in practical applications. In contrast to elliptical ones, Archimedean
 copulas are given explicitly in terms of a generator and they are able to
 ...
 different magnitude. With the algorithm of \citet{marshallolkin1988}, they are
...
exchangeability, Archimedean copulas can be nested within each other under
certain conditions. The resulting copulas are referred to as ``nested Archimedean copulas'' and allow to model hierarchical dependence structures.
...
Archimedean and nested Archimedean copulas. In contrast to other \R\ packages
 dealing with Archimedean copulas, e.g., \pkg{copula} or \pkg{fCopulae},
...
generators are the ones given in \citet[pp.\ 116]{nelsen2007}. Also note that for the families of Ali-Mikhail-Haq, Frank, and Joe, $F$ is discrete with support $\IN$. In this case, $\psi$ is of the form $\psi(t)=\sum_{k=1}^\infty p_k\exp(-kt)$, $t\in[0,\infty]$, where $(p_k)_{k=1}^\infty$ denotes the mass function corresponding to $F$.
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{\extracolsep{\fill}}X>{\hspace{-6mm}}cc>{\hspace{-4mm}}c}
    \toprule
    \multicolumn{1}{c}{Family}&\multicolumn{1}{c}{\hspace{-6mm}$\theta$}&\multicolumn{1}{c}{$\psi(t)$}&\multicolumn{1}{c}{\hspace{-4mm}$V\sim F=\LSi[\psi]$}\\
    \midrule
    Ali-Mikhail-Haq&$[0,1)$&$(1-\theta)/(\exp(t)-\theta)$&$\Geo(1-\theta)$\\
    Clayton&$(0,\infty)$&$(1+t)^{-{1/\theta}}$&$\Gamma(1/\theta,1)$\\
    Frank&$(0,\infty)$&$-\log(1-(1-e^{-\theta})\exp(-t))/\theta$&$\Log(1-e^{-\theta})$\\
    Gumbel&$[1,\infty)$&$\exp(-t^{1/\theta})$&$\S(1/\theta,1,\cos^\theta(\pi/(2\theta)),\I_{\{\theta=1\}};1)$\\
    Joe&$[1,\infty)$&$1-(1-\exp(-t))^{1/\theta}$&$\binom{1/\theta}{k}(-1)^{k-1},\ k\in\IN$\\
    \bottomrule
  \end{tabularx}
...
$p_k=\binom{\alpha}{k}(-1)^{k-1}$ at $k\in\IN$ ($\alpha=1/\theta$), with
$\alpha\in(0,1]$; $\binom{\alpha}{k}=\alpha*(\alpha-1)*\dots*(\alpha-k+1)/k!$
 denotes the (generalized) binomial coefficient, which is implemented in \R\ via
 \code{choose(}$\alpha$,$k$\code{)}. This distribution can be sampled via the
 \R{} function \code{rFJoe(., }$\alpha$\code{)} which we implemented based on an
 algorithm presented in \citet{hofert2011a}.
...
i.e., $D_1(\theta)=\frac 1 \theta \int_0^\theta t/(e^t-1)\,dt$, and for Joe's family, it
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{\extracolsep{\fill}}Xcccc}
    \toprule
    \multicolumn{1}{c}{Family}&\multicolumn{1}{c}{$\theta$}&\multicolumn{1}{c}{$\tau$}&\multicolumn{1}{c}{$\lambda_l$}&\multicolumn{1}{c}{$\lambda_u$}\\
    \midrule
    Ali-Mikhail-Haq& $[0,1)$      &$1-2(\theta+(1-\theta)^2\log(1-\theta))/(3\theta^2)$ & 0 & 0\\
    Clayton        & $(0,\infty)$ &$\theta/(\theta+2)$		&$2^{-1/\theta}$ & 0\\
    Frank          & $(0,\infty)$ &$1+4(D_1(\theta)-1)/\theta$	& 0	& 0\\
    Gumbel         & $[1,\infty)$ &$(\theta-1)/\theta$		& 0	&$2-2^{1/\theta}$\\
    Joe            & $[1,\infty)$ &$1-4\sum_{k=1}^\infty 1/(k(\theta k+2)(\theta(k-1)+2))$
								& 0	&$2-2^{1/\theta}$\\
    \bottomrule
  \end{tabularx}
...
is a version of standard's \pkg{lattice} \code{splom()}, with the addition
...
splom2(U3, cex = 0.4)
<<ex1-splom, echo=FALSE, fig=TRUE, height=5>>=
print(
<<ex1-splom-def>>
     )
@
...
other. In practical applications, this hierarchical structure is often naturally
motivated, e.g., by different macroeconomic effects, political decisions, or
consumer trends affecting the components of a random vector.
...
This \code{childCops} list is the tool to contain sub branches of the ``tree''
of nested copulas, and will be empty, i.e., $=$\code{list()}, for the final
nodes (``leaves'') of the tree. In the example, it will be of length one, containing
a (non-nested) bivariate copula for components $u_2$ and $u_3$.

The \code{outer_nacopula} is a version of the same class (i.e., it
...
The \code{onacopula()} function\footnote{in addition to \code{onacopula()},
  there's \code{onacopulaL()} (``L'' for \textbf{L}ist) which requires the
...
the nesting structure \emph{and} parameters for each copula, using a ``formula''-like
notation $C(\theta, c(i_1,\dots,i_c), list(C(..), \ldots, C(..)))$ similar
...
\begin{equation*}
  \psi_{ij}(t;x)=\exp\bigl(-x\psiis{i}(\psi_{j}(t))\bigr),\ x\in(0,\infty).
\end{equation*}
...
sufficient nesting condition if $\psi_i,\psi_j$ belong to different Archimedean
families is usually more complicated, see, \citet{hofert2010c}. Such
...
\citet{hofert2011a}, and \citet{hofert2010c}. For the families of
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{\extracolsep{\fill}}X>{\hspace{-7mm}}c>{\hspace{-8mm}}c}
	...
	\midrule
	Ali-Mikhail-Haq&$\theta_0\le\theta_1$&$V_0+X$, $X\sim\NB(V_0,(1-\theta_1)/(1-\theta_0))$\\
	Clayton&$\theta_0\le\theta_1$&$\tS(\theta_0/\theta_1,1,(\cos(\pi\theta_0/(2\theta_1))V_0)^{\theta_1/\theta_0},V_0\I_{\{\theta_0=\theta_1\}},\I_{\{\theta_0\neq\theta_1\}};1)$\\
	Frank&$\theta_0\le\theta_1$&$\sum_{j=1}^{V_0}V_j$; for $V_j$, see \citet{hofert2011a}\\
	Gumbel&$\theta_0\le\theta_1$&$\S(\theta_0/\theta_1,1,(\cos(\pi\theta_0/(2\theta_1))V_{0})^{\theta_1/\theta_0},\I_{\{\theta_0=\theta_1\}};1)$\\
	Joe&$\theta_0\le\theta_1$&$\sum_{j=1}^{V_0}V_j$; $V_j\sim\binom{\alpha}{k}(-1)^{k-1},\ k\in\IN$\\
	\bottomrule
   \end{tabularx}
...
For Frank's family, \citet{hofert2011a} presented a rejection algorithm.
For the family of Gumbel, $\psi_{01}(t;V_0)=\exp(-V_0t^\alpha)$,
$\alpha=\theta_0/\theta_1$, hence, see Table~\ref{tab:cop-families-psi}, $F_{01}$ corresponds
...
see \citet{hofert2011a} and \citet{hofert2010c}, or the source code of
...
<<ex2-splom, echo=FALSE, fig=TRUE, height=7>>=
print(
<<ex2-splom-def>>
      )
@
...
condition if $\theta_0\le\theta_1$. Therefore, one can build so-called
...
splom2(U3, cex = 0.4)
<<opower-splom, echo=FALSE, fig=TRUE, height=5>>=
print(
<<opower-splom-def>>
      )
@
