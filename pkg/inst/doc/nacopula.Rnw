...
\end{align}
Conversely, give a copula $C$ and any univariate distribution functions $F_j$,
$j\in\{1,\dots,d\}$, $H$ defined by (\ref{sklar}) is a distribution function
with marginals $F_j$, $j\in\{1,\dots,d\}$. On the one hand, Sklar's Theorem
...
tool for constructing multivariate distributions and is therefore often used for
sampling multivariate distributions via copulas. This is indispensable for
...
Besides elliptical copulas, Archimedean
...
typically high-dimensional. To circumvent this symmetry, Archimedean copulas
