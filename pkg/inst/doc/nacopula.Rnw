...
@
In general, parametric\emph{nested} Archimedean copulas
It
...
Figure~\ref{fig:NAC_3d} above is
<<<NAC_3d-ex>>=
...
objects therefore provide the two slots \code{nestConstr} and \code{V01},
both functions. The first, \code{nestConstr}, is a
\code{function(}$\theta_0$\code{, }$\theta_1$\code{)} returning \code{TRUE}
when the nesting condition is fulfilled, and \code{V01} is random number
generating function, generating $V_{01}$ from Table~\ref{tab:nestedAC-V01},
...
@
<<nacopula-version>>=
packageDescription("nacopula")[["LastChanged"]]
@
