...
$p_k=\binom{\alpha}{k}(-1)^{k-1}$ at $k\in\IN$ (take $\alpha:=1/\theta$), with
...
generated data. The former should be approximately 0.5, since we chose the parameter $\theta$ accordingly.
<<ex1-explore1>>=
copJoe @ tau(theta)
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{\extracolsep{\fill}}X>{\hspace{-7mm}}c>{\hspace{-8mm}}c}
	...
	Frank&$\theta_0\le\theta_1$&$\sum_{j=1}^{V_0}V_j$; for $V_j$, see \citet{hofert2010a}\\
...
\citet{hofert2010a} is more often applied if $\theta_0>1/4$,
