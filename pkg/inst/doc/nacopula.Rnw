...
%% almost as usual
\author{Marius Hofert \\ETH Zurich \And
...
require(nacopula)
## find the parameter theta of a Clayton copula such that Kendall's tau equals 0.5
...
print(splom(~ U3, varnames = paste("U", 1:3, sep="_")))
@
...
round(cor(U3, method="kendall"), 3) # off-diagonal entries should be close to .5
...
sampling nested Archimedean copulas, see \textcite{hofert2010e}.
\begin{algorithm}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{\extracolsep{\fill}}X>{\hspace{-7mm}}c>{\hspace{-8mm}}c}
	...
	\bottomrule
   \end{tabularx}
   \caption{Nesting conditions and inverse Laplace-Stieltjes transforms corresponding to the
...
the package \pkg{nacopula}.
 ...
 \subsection{A working example}
 Let $C$ be a nine-dimensional partially nested Archimedean copula of the form
 \begin{align*}
   C(\bm{u})=C(u_1,u_3,u_6,C(u_2,u_5,u_7,u_9,C(u_4,u_8;\psi_2);\psi_1);\psi_0)
 \end{align*}
 with tree structure depicted in Figure \ref{15}.
 \begin{figure}[htbp]
   \centering
   \begin{tikzpicture}[
		grow=south,
  ...
  \end{tikzpicture}
  \setcapwidth{\textwidth}%
  \caption{Tree structure of the nine-dimensional partially nested Archimedean copula $C$.}
  \label{figure2}
\end{figure}

todo marius: 9d nested Clayton copula beschreiben
todo mcneil algo wieder einbauen
todo lambdas im code
todo *alles* committen (auch bib file)
todo \footnote{The author (Willis Research Fellow) thanks Willis
    Re for financial support while this paper was completed.}
