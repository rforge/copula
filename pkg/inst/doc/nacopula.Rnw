...
%% almost as usual
\author{Marius Hofert\thanks{The author (Willis Research Fellow) thanks Willis
    Re for financial support while this paper was completed.} \\ETH Zurich \And
 Martin M\"achler \\ ETH Zurich}
\title{Nested Archimedean Copulas---The R package \pkg{nacopula}}

%% for pretty printing and a nice hypersummary also set:
\Plainauthor{Marius Hofert, Martin M\"achler} %% comma-separated
\Plaintitle{Nested Archimedean Copulas---The R package nacopula} %% without formatting
\Shorttitle{Nested Archimedean Copulas---\pkg{nacopula}}
...
\Abstract{
Sampling algorithms for multivariate distributions are indispensable for many
applications in the areas of statistics and finance. By Sklar's Theorem, it
suffices to sample the common dependence structure, given by a copula, and
transform the obtained variates to the correct margins with the corresponding
univariate distributions. Therefore, the sampling problem usually boils down to
sampling the copula under consideration. Besides elliptical copulas,
Archimedean copulas are among the most important classes of copulas. These 
copulas can be generalized to the class of nested Archimedean copulas, which
allows for hierarchical structures, i.e., asymmetries, and is therefore more
flexible. Hierarchical structures of the kind implied by nested Archimedean
copulas often appear in models for high-dimensional portfolios, e.g., in
risk-management applications.
\par
The \pkg{nacopula} package provides procedures for constructing arbitrary
nested Archimedean copulas (with any kind of nesting structure in any
dimension), generating vectors of random variates from nested Archimedean
copulas, computing function values and probabilities of falling into hypercubes,
as well as evaluation of characteristics such as Kendall's tau and
tail-dependence coefficients. As a by-product, algorithms for different
distributions, including exponentially tilted stable distributions, are
implemented. Detailed examples are given.
}
...
\newcommand*{\I}{\mathbb{I}} %todo: \newcommand*{\I}{\mathbbm{1}} would be nicer
...
uniform univariate margins. Standard references are \textcite{joe1997}
and \textcite{nelsen2007}. Besides elliptical copulas, Archimedean
copulas play an important role in practical applications. In contrast to
elliptical copulas, Archimedean copulas are given explicitly in terms of
generator and they are able to capture different kinds of tail dependencies,
e.g., upper, but not lower tail dependence. With the algorithm of
\textcite{marshallolkin1988}, they are also usually easy to sample. Their
functional symmetry, however, is often considered to be a drawback, e.g.,
in risk-management applications where the considered portfolios are
often high-dimensional. To circumvent this symmetry, Archimedean copulas
can be nested into each other under certain conditions. The resulting
copulas are referred to as nested Archimedean copulas and allow to model
hierarchical dependence structures. One of the first appearances of these
copulas in the literature is \textcite[pp.\ 87]{joe1997}.

\section{Archimedean copulas in the package}
...
\textcite{kimberling1974} or \textcite[p.\ 54]{hofert2010c}. The class of
all completely monotone Archimedean generators is denoted by $\Psi_\infty$
in what follows. By Bernstein's Theorem, see, e.g.,
\textcite[p.\ 439]{feller1971}, this class coincides with the class of
Laplace-Stieltjes transforms of distribution functions on the positive
real line, i.e., for a $\psi\in\Psi_\infty$, we have
\begin{align*}
  \psi=\LS[F],\ \text{or, equivalently,}\ F=\LSi[\psi].
\end{align*}
for a distribution function $F$ on the positive real line.
\par
With a mixture representation with respect to $F$, the following algorithm
...
\begin{algorithm}[\textcite{marshallolkin1988}]
  ...
  \begin{tabbing}
    ...
    (2)	\> \textbf{sample} $X_j\sim\U[0,1]$, $j\in\{1,\dots,d\}$\\
    (3)	\> \textbf{set} $U_j:=\psi(-\log(X_j)/V)$, $j\in\{1,\dots,d\}$\\
    (4) \> \textbf{return} $\bm{U}=(U_1,\dots,U_d)^T$
  \end{tabbing}
\end{algorithm}
...
\pkg{nacopula} package currently allows for the families of Ali-Mikhail-Haq,
Clayton, Frank, Gumbel, and Joe, see Table \ref{table1} for the implemented
generators of these Archimedean families and their corresponding
distribution functions. Except for Clayton's family, where we use a slightly
simpler generator, these generators are also given in
\textcite[pp.\ 116]{nelsen2007}.
\begin{table}[htbp]
  \centering
...
$p_k=p(1-p)^{k-1}$ at $k\in\IN$. For Clayton's family,
$\Gamma(\alpha,\beta)$ denotes the \textit{gamma distribution} with shape
$\alpha\in(0,\infty)$, scale $\beta\in(0,\infty)$, and density
$f(x)=\beta^\alpha x^{\alpha-1}\exp(-\beta x)/\Gamma(\alpha)$,
$x\in(0,\infty)$. For the family of Frank, $\Log(p)$ denotes a
\textit{logarithmic distribution} with parameter $p\in(0,1)$ and mass
function $p_k=p^k/(-k\log(1-p))$ at $k\in\IN$. For sampling this
distribution, we implemented algorithm ``LK'' of \textcite{kemp1981}. For
Gumbel's family, $F$ corresponds to a stable distribution, see
\textcite[p.\ 8]{nolan2009} for the parameterization. We implemented an
algorithm for sampling stable distributions according to the ideas presented
in \textcite{chambersmallowsstuck1976}. For the family of Joe,
$p_k=\binom{1/\vt}{k}(-1)^{k-1}$ at $k\in\IN$, with $\vt\in[1,\infty)$. This
distribution can be sampled with an algorithm found in
\textcite{hofert2010a}.
\par
It is often desirable to measure the degree of dependence between random
variables by a real number. Such measures are referred to as
\textit{measures of association} and are usually studied for pairs of random
variables. One such measure is \textit{Kendall's tau}, defined by
\begin{align*}
  \tau:=\IE[\sgn((X_1-X_1^\prime)(X_2-X_2^\prime))],
\end{align*}
where $(X_1,X_2)^T$ is a vector of two continuously distributed random
variables, $(X_1^\prime,X_2^\prime)^T$ is an independent and identically
distributed copy of $(X_1,X_2)^T$, and
$\sgn(x):=\I_{(0,\infty)}(x)-\I_{(-\infty,0)}(x)$. Kendall's tau is a
measure of concordance, see \textcite{scarsini1984}, and therefore,
...
\begin{align*}
  \tau=4\int_0^1\frac{\psii(t)}{(\psii(t))^\prime}\,dt+1=1-4\int_0^\infty t(\psiD(t))^2\,dt,
\end{align*}
...
i.e., $D_1(\vt):=\int_0^\vt t/(\exp(t)-1)\,dt/\vt$, and for Joe's family, it
is given as a series, see Table \ref{table2}.
\par
Tail dependence measures the probability that one random variable takes on
values in its tail, given the other one takes on values in its tail. To be
more precise, if $X_j\sim F_j$, $j\in\{1,2\}$, are continuously distributed
random variables, the \textit{lower tail-dependence coefficient},
respectively the \textit{upper tail-dependence coefficient}, of $X_1$ and
$X_2$ are defined as
\begin{align*}
...
expressed in terms of the copula $C$ corresponding to $(X_1,X_2)^T$. If $C$
...
implemented Archimedean families, these limits can easily be found. They are
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{\extracolsep{\fill}}Xcccc}
    ...
    Joe&$[1,\infty)$&$1-4\sum_{k=1}^\infty 1/(k(\vt k+2)(\vt(k-1)+2))$&0&$2-2^{1/\vt}$\\
    \bottomrule
  \end{tabularx}
...
require(nacopula)
## find the parameter theta of a Clayton copula such that Kendall's tau 
## equals 0.5
(theta <- copClayton@tauInv(0.5))

## generate a 3d Clayton copula with parameter theta
claytonCop <- onacopula("Clayton", C(theta, 1:3))
str(claytonCop)
@ % end code

Now, we sample 500 random variates (each in $[0,1]^3$) from this copula
and visualize them in a scatter-plot matrix:
<<splom-ex1, fig=TRUE, height=5>>=
set.seed(1)
dim(U3 <- rnacopula(500, claytonCop))
print(splom(~ U3, varnames = paste("U", 1:3, sep="_"), cex = 0.4))
@
%% "FIXME" : maybe use something like our  splom2 in the future

Looking at different properties of the copula happens by function calls to
appropriate methods:
<<explore-ex1>>=
## evaluate this copula at (.5, .5, .5) and at almost the outermost corner:
c(pnacopula(claytonCop, c(.5, .5, .5)),
  pnacopula(claytonCop, c(.99,.99,.99)))

## What's the probability mass in the cube  (.8, 1]^3 :
prob(claytonCop, c(.8, .8, .8), c(1, 1, 1))
## compare population and sample version of Kendall's tau
copClayton @ tau(theta) # 0.5 because theta was chosen in this way !
round(cor(U3, method="kendall"), 3) # off-diagonal entries should be close
												# to .5

## the lower and upper tail-dependence coefficients:
c(copClayton @ lambdaL(theta),
  copClayton @ lambdaU(theta))
@


\section{Nested Archimedean copulas in the package}
...
explicitly, see (\ref{ac}), hence function evaluations are usually easy to
establish. However, the functional symmetry of Archimedean copulas implies that
all margins of the same dimension are equal. For modeling purposes, this
becomes an increasingly strong assumption in the dimension. Asymmetries, thus
...
tree, see Figure \ref{figure1} for the three-dimensional case of Type
...
\textit{leaf copulas}. Further, we call an Archimedean copula appearing in a
nested Archimedean copula \textit{parent copula} if it is a nested
Archimedean copula with at least one nested Archimedean copula as one of its
components. We refer to these components as \textit{child copulas}.
\par
In order for (\ref{nac}) being indeed a proper copula,
\textcite[p.\ 88]{joe1997} and \textcite{mcneil2008} present the sufficient
\textit{nesting condition} that $\psiis{i}\circ\psi_{j}$ have completely
monotone derivatives for all nodes appearing in a nested Archimedean copula.
...
$x\in(0,\infty)$. If this condition is satisfied for all appearing nodes in
the nested Archimedean structure, the following algorithm may be derived for
...
\begin{algorithm}
  ...
  \begin{tabbing}
   ...
   \end{tabbing}
\end{algorithm}
Note that for sampling the nested Archimedean copulas when all generators
...
the parameters may differ. Sampling strategies for $F_0$ and $F_{01}$ for
...
Ali-Mikhail-Haq, Clayton, Frank, Gumbel, and Joe, Table \ref{table3}
...
\begin{table}[htbp]
  ...
  \begin{tabularx}{\textwidth}{@{\extracolsep{\fill}}X>{\hspace{-7mm}}c>{\hspace{-8mm}}c}
	...
	\bottomrule
   \end{tabularx}
   \caption{Nesting conditions and inverse Laplace-Stieltjes transforms
corresponding to the
     generators $\psi_{01}(\cdot\,;V_0)$.}
   \label{table3}
\end{table}
\par
First note that on staying within the same Archimedean family, the
implemented families lead to proper nested Archimedean copulas if the
generators on a more nested (inner) level have larger parameter values than
the ones on a lower (outer) level. This is equivalent to saying that
Kendall's tau for two pairs of random variables having an Archimedean copula
which resides on a deeper nesting level has to be larger than or equal to
the one for two pairs of random variables having an Archimedean copula
residing on a lower nesting level.
\par
Now let us comment on the distributions $F_{01}$ of $V_{01}$ for the
...
binomial distribution with parameters as given in Table \ref{table3}. The
parameterization is $\NB(r,p)$, $r\in(0,\infty)$, $p\in(0,1)$, with mass
function $p_k=\binom{k+r-1}{r-1}p^r(1-p)^k$, $k\in\IN_0$, as implemented in
\proglang{R}.
\par
...
distribution. \textcite{devroye2009} recently suggested an algorithm for
...
Laplace-Stieltes transform
\begin{align*}
  \psi(t)=\exp(-(\lambda+t)^{\alpha}-\lambda^{\alpha}),\ t\in[0,\infty].
\end{align*}
Only can easily check that by setting $\lambda:=hV_0^{1/\alpha}$ and
...
obtained as $V_{01}=V_0^{1/\alpha}V_\lambda$. Therefore, the distribution as
...
\pkg{nacopula} and decide upon sampling $V_{01}$ which method is to be
applied. As a simple rule, investigated by several parameter combinations,
the default chooses the method of \textcite{devroye2009} if
$V_{0}h^{\alpha}\ge4$ and the one of \textcite{hofert2010a} otherwise. As
mentioned before, note that for sampling a nested Clayton copulas, we have
$h=1$. Further, $\IE[V_0]=1/\vt_0$. Hence, in the mean, the algorithm of
\textcite{devroye2009} is more often applied if $\theta_0\le1/4$,
equivalently, if Kendall's tau for the bivariate Archimedean copula
generated by $\psi_0$ is less than or equal to $1/9$.
\par
...
$\alpha:=\vt_0/\vt_1$, hence, see Table \ref{table1}, $F_{01}$ corresponds
to the stable distribution as given in Table \ref{table3}. Finally, for
...
Table \ref{table1}.	For more information on the distributions of $V_{01}$,
see \textcite{hofert2010a} and \textcite{hofert2010c}, or the source code of
the package \pkg{nacopula}.

\subsection{A working example}
In this example, we consider nine-dimensional partially nested Archimedean
...
require(nacopula)
## choose the parameters theta0, theta1, and theta2 this nested Clayton
## copula such that the Kendall's tau equal 0.2, 0.5, 0.8
(theta0 <- copClayton@tauInv(0.2))
(theta1 <- copClayton@tauInv(0.5))
(theta2 <- copClayton@tauInv(0.8))

## generate the 9d nested Clayton copula (this is the copula in one of 
## the examples in the file ./tests/nac-experi.R)
claytonCop <- onacopula("Clayton", C(theta0, c(3,6,1),
												 C(theta1,c(9,2,7,5),
												   C(theta2, c(8,4)))))
str(claytonCop)
@ % end code

Now, we sample 500 random variates (each in $[0,1]^9$) from this copula
(involves our efficient procedure for exponentially tilted stable distributions)
and visualize them in a scatter-plot matrix:
<<splom-ex2, fig=TRUE, height=9>>=
set.seed(1)
dim(U9 <- rnacopula(500, claytonCop))
print(splom(~ U9, varnames = c("U_3", "U_6", "U_1", "U_9", "U_2", "U_7",
			  	"U_5", "U_8", "U_4"), cex = 0.4, 
...
appropriate methods:
<<explore-ex2>>=
## evaluate this copula at (.5,...,.5) and at almost the outermost corner:
c(pnacopula(claytonCop, rep(.5,9)),
  pnacopula(claytonCop, rep(.99,9)))

## What's the probability mass in the cube  (.8, 1]^9 :
prob(claytonCop, rep(.8,9), rep(1,9))
## What's the population and sample version of Kendall's tau 
## for the pair (U_4,U_5)?
round(cor(U9[,9],U9[,7], method="kendall"), 3) # should be close to .5

## the different lower and upper tail-dependence coefficients appearing in
## copClayton:
...
todo marius splom 9d plot: remove ticks etc.
todo code not properly indented on page 11
todo warning in aux-acopula.R => V0 vector! => muss vector von methods sein
todo Martin: introduce classes, technical stuff important for the paper
%%   =======
...
\section{conclusion}
The package \pkg{nacopula} allows to easily construct and and work with nested
Archimedean copulas. First and foremost, sampling algorithms for these copulas
are implemented. As a by-products, the package implements, e.g., an efficient
...
the one of Ali-Mikhail-Haq, Clayton, Frank, Gumbel, and Joe.
