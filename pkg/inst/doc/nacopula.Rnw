...
\Abstract{
  Sampling algorithms for multivariate distributions are indispensable for
  many applications in the areas of statistics and finance.  By Sklar's
  Theorem, it suffices to sample the common dependence structure, given by
  a copula, and to transform the obtained variates to the correct marginals
  with the corresponding univariate distributions.  Therefore, sampling
  from such multivariate distributions usually boils down to sampling the
  copula under consideration.  Besides elliptical copulas, Archimedean
  copulas are among the most important classes of copulas. These copulas
  can be generalized to the class of nested Archimedean copulas, which
  allow for hierarchical structures, and hence, asymmetries, and are
  therefore more flexible. Such hierarchical structures often appear in
  models for high-dimensional portfolios, e.g., in risk-management
  applications.
  ...
  dimension, generating vectors of random variates from the constructed
...
\newcommand*{\I}{\mathbf{1}}% \mathbb{1} gibt nonsens -- FIXME Marius(?)
...
Recall that the most simple model, namely, independence, is provided by
$\psi(t) = e^{-t}$, $psii(t)=log(t)$, $C(\bm(u)) = \prod_{j=1}^d u_j$.
...
real line, where the Laplace-Stieltjes transform of a distribution function
$F$, in short, the Laplace transform of the distribution, is defined as
%% TODO: Marius is this "perfect"?
\begin{equation}
  \LS[F](u) := \int e^{-ut}dF(t), \ \ \ u \in \IR.\label{def:LS}
\end{equation}
vFor a $\psi\in\Psi_\infty$, we hence have
\begin{align*}
...
\pkg{nacopula} package currently provides the most widely used families of
Ali-Mikhail-Haq, Clayton, Frank, Gumbel, and Joe, see Table~\ref{table1}
...
objects, containing as slots the corresponding
$\psi()$ function, \code{psi}, its inverse $\psii()$, \code{psiInv},
and the ``sampler'', i.e., random number generator for $V ~ F$, as \code{V0}:
<<acopula-family>>=
require(nacopula)
...
@
where most slots of such copula objects are functions, encoding properties
of that copula, most of which we are considering subsequently.

\subsubsection{Sampling from Archimedean copulas}
...
%% TODO: Ist die Tabelle hier immer noch am richtigen Ort?
%% ----  wohl schon, jedenfalls müssen die Erklärungen unten gerade anschliessen?
\begin{table}[htbp]
  \centering
...
%
For Gumbel's family, $F$ corresponds to a stable distribution,
%% TODO: Marius, say a bit more
$S(\alpha, \beta, \gamma, \delta)$  in ``parametrization 1'' --- FIXME Marius ---
\par\noindent
%% end{TODO}
see \textcite[p.\ 8]{nolan2009} for more details.

For sampling from $S(.)$, we provide
...
%
For the family of Joe, $p_k=\binom{1/\vt}{k}(-1)^{k-1}$ at $k\in\IN$, with
$\vt\in[1,\infty)$.  This distribution can be sampled
%% via \R function \code{rFJoe(., $\alpha$)}, -- nein, FIXME
%% implemented
with an algorithm found in \textcite{hofert2010a}.

\subsubsection{Kendall's tau (rank correlation coefficient)}
...
distributed copy of $(X_1,X_2)\tr$, and the usual signum function
$\sign(x):=\I_{(0,\infty)}(x)-\I_{(-\infty,0)}(x)$ (as \R's
\code{sign(x)}). Kendall's tau is a measure of concordance, see
...
is given as a series in Table~\ref{table2}.
...
As a working example, we define a three-dimensional Joe copula with
...
claytonCop <- onacopula("Joe", C(theta, 1:3))
@
...
generated data.
<<ex1-explore1>>=
copJoe @ tau(theta) # ~= 0.5 because theta was chosen such!
round(cor(U3, method="kendall"), 3)
@

Next, let us evaluate this Joe copula at $(0.5,0.5,0.5)\tr$ and near the
...
For now, we use the outer power transformation \texttt{opower}
which generates an outer power family based on the provided copula family
\texttt{copbase} with corresponding parameter \texttt{thetabase}.
<<outerpower-def>>=
str(opower)
@
...
\code{theta}$=\vartheta$ will become 2-dimensional), rather than using the
\code{opower()} construction below. This section therefore should be
considered mainly as an outlook to further features of the package
\pkg{nacopula}, where the typical use will change in the future.
...
<<opower-notsupp>>=
if(FALSE){## currently not supported

  ## Define a 3d fully nested Archimedean copula based on  opClayton, from above
  opC3 <- onacopula(opClayton, C(theta0, 1, C(theta1, c(2,3))))
