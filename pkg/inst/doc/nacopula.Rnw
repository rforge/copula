...
%% almost as usual
\author{Marius Hofert\footnote{The author (Willis Research Fellow) thanks Willis
 Re for financial support while this paper was completed.}\\ETH Zurich \And
 Martin M\"achler\\ETH Zurich}%todo: footnote does not appear
...
\SweaveOpts{engine=R,eps=FALSE,pdf=TRUE,width=7,height=4,strip.white=TRUE,keep.source=TRUE} %% from Martin
...
\Abstract{
 ...
 univariate distributions. Therefore, the sampling problem boils down to
 sampling the copula under consideration. Besides elliptical copulas,
 Archimedean copulas are among the most famous classes of copulas. These copulas
 can be generalized to the class of nested Archimedean copulas, which allows for
 hierarchical structures, i.e., asymmetries, and is therefore more flexible.
 Hierarchical structures of the kind implied by nested Archimedean copulas often
 appear in models for high-dimensional portfolios, e.g., in risk-management
 applications.
  \par
  The \pkg{nacopula} package provides procedures for constructing arbitrary
 nested Archimedean copulas (with any kind of nesting structure in any kind of
 dimension), generating vectors of random variates from nested Archimedean
 copulas and drawing scatter-plot matrices, computing function values and
 probabilities of falling into hypercubes, as well as evaluation of
 characteristics such as Kendall's tau and tail-dependence coefficients. As a
 by-product, algorithms for different distributions, including exponentially
 tilted stable distributions, are implemented. Detailed examples are given.
}
...
\section{Archimedean copulas in the package}
 ...
 \textit{Archimedean} if it is of the form
	...
	## evaluate this copula at (.5, .5, .5)
	pnacopula(claytonCop, c(.5, .5, .5))
	
	## find the mass in the cube (.8, 1]^3
	prob(claytonCop, c(.8, .8, .8), c(1, 1, 1))	
	
	## sample 1000 random variates from this copula and draw a scatter-plot matrix
	data <- rnacopula(1000, claytonCop)
	spm(data)
	
	## compare population and sample version of Kendall's tau 
	copClayton@tau(theta) # should be .5 because theta was chosen in this way
	cor(data, method="kendall") # the entries (1,2), (1,3), and (2,3) should be close to .5
	
	## compute the lower and upper tail-dependence coefficient for this copula
	copClayton@lTDC(theta) 
	copClayton@uTDC(theta)
...
\section{Nested Archimedean copulas in the package}
 ...
 up to permutation of the arguments, by
	\begin{align}
		C(\bm{u};\psi_0,\dots,\psi_{d-2}):=\psi_0\bigl(\psiis{0}(u_1)+\psiis{0}(C(u_2,\dots,u_d;\psi_{1},\dots,\psi_{d-2}))\bigr),\ \bm{u}\in[0,1]^d,\label{nac}
	\end{align}		
  ...
  \end{figure}
	...
	\begin{table}[htbp]
		...
		\begin{tabularx}{\textwidth}{@{\extracolsep{\fill}}X>{\hspace{-7mm}}c>{\hspace{-8mm}}c}
			\toprule
			\multicolumn{1}{c}{Family}&\multicolumn{1}{c}{\hspace{-7mm}nesting condition}&\hspace{-8mm}$V_{01}\sim F_{01}=\LSi[\psi_{01}(\cdot\,;V_0)]$\\
			\midrule
			...
			\bottomrule
		\end{tabularx}
		\caption{Nesting conditions and inverse Laplace-Stieltjes transforms corresponding to the generators $\psi_{01}(\cdot\,;V_0)$.}
		\label{table3}
	\end{table}
	\par
