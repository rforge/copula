...
\title{Nested Archimedean Copulas meet R---The package \pkg{nacopula}}
% other possible titles:
%\title{Nested Archimedean Copulas + R = \pkg{nacopula}}
...
suffices to sample the common dependence structure, given by a copula, and to
transform the obtained variates to the correct marginals with the corresponding
...
flexible. Such hierarchical structures often appear in models for
high-dimensional portfolios, e.g., in risk-management applications.
\par
The \pkg{nacopula} package provides procedures for constructing arbitrary
nested Archimedean copulas with any kind of nesting structure and in any
dimension, generating vectors of random variates from the constructed objects,
computing function values and probabilities of falling into hypercubes,
as well as evaluation of characteristics such as Kendall's tau and the
tail-dependence coefficients. As a by-product, algorithms for various
...
a generator and they are able to capture different kinds of tail dependencies,
...
\textcite[p.\ 54]{hofert2010c} for a rework in terms of copulas. The class of
...
\pkg{nacopula} package currently allows for the most widely used families of
Ali-Mikhail-Haq, Clayton, Frank, Gumbel, and Joe, see Table \ref{table1} for the
implemented generators and their corresponding distribution functions. Except
for Clayton's family, where we use a slightly simpler generator, these
generators are also given in \textcite[pp.\ 116]{nelsen2007}.
\begin{table}[htbp]
  \centering
...
distribution, we implemented the algorithm ``LK'' of \textcite{kemp1981}. For
...
also given in Table \ref{table2}.
\begin{table}[htbp]
  \centering
  ...
  \label{table2}
\end{table}
\subsection{A three-dimensional Clayton copula}
As a working example, we define a three-dimensional Clayton copula with
...
Now we display the structure of this object.
<<ex1-str>>=
str(claytonCop) # display the structure of this object
@ 
...
@
%% "FIXME" : maybe use something like our  splom2 in the future (everywhere)
...
Next, let us evaluate this Clayton copula at $(0.5,0.5,0.5)^T$ and near the
upper corner $(0.99,0.99,0.99)^T$.
<<ex1-explore2>>=
...
\subsection{Construction and sampling}
...
model. However, the functional symmetry of Archimedean copulas implies that
...
$x\in(0,\infty)$. If this condition holds true for all appearing nodes in
...
copula if the generators on a more nested (inner) level have larger parameter
...
\end{align*}
with tree structure depicted in Figure \ref{figure2}. This copula may also be
found in the file \texttt{./tests/nac-experi.R} of the package \pkg{nacopula}.
\begin{figure}[htbp]
 \centering
 \begin{tikzpicture}[
	grow=south,
...
require(lattice)
print(splom(~ U9, varnames = paste("U", 1:9, sep="_"), cex = 0.4, 
				pscales = 0)) 
@

The population version of Kendall's tau for $(U_4,U_5)^T$ is 0.5. Let us check
...
$(0.5,\dots,0.5)^T$ and near the upper corner $(0.99,\dots,0.99)^T$ leads to 
...
condition if $\vt_0\le\vt_1$. Therefore, one can build \textit{nested outer
power Archimedean copulas}. \textcite{hofert2010a} derives some results for
...
Archimedean copulas with the package \pkg{nacopula}. Note that these copulas are
currently not supported by the package. This section therefore should be
considered as an outlook of possible features of the package \pkg{nacopula} in
the future. The following code provides the details.
\par 
First, we define the outer power transformation \texttt{op} which generates an
outer power family based on the provided copula family \texttt{copbase} with
corresponding parameter \texttt{thetabase}.
<<ex3-definition>>=
require(nacopula)
op <- function(copbase, thetabase){
    new("acopula", name = paste("op", copbase@name, sep=""),
        ## generator
        ...
        lambdaL = function(theta) { 
            if(copbase@name=="Clayton") 2^(-1/(thetabase*theta))
            else 0*theta	
	},
        ...
        lambdaU = function(theta) { 
            if(copbase@name=="Gumbel" | copbase@name=="Joe"){
...
opClayton <- op(copClayton, thetabase) 
@
...
such that Kendall's tau are 0.2 and 0.5.
<<ex3-definition3>>= 
theta0 <- opClayton@tauInv(2/3) # should be 1.5
theta1 <- opClayton@tauInv(.75) # should be 2
@

Note that the following chunk is currently not supported.
<<ex3-notsupp>>= 
## currently not supported (start) 
if(FALSE){

    ## Define a 3d fully nested Archimedean copula based on the opClayton
 	 ## defined as above
    opC <- onacopula("opClayton", C(theta0, 1, C(theta1, c(2,3)))) 

    ## Sample 500 random variates from this copula and plot the data
    dim(U3 <- rnacopula(500, opClayton))
    splom(~ U3, varnames = paste("U", 1:3, sep="_"), cex = 0.4)

}
## currently not supported (end)
@

We therefore construct random variates from this copula as follows.
<<ex3-construct>>= 
set.seed(1)
## (1) draw V0 and V01
V0 <- opClayton@V0(500, theta0)
V01 <- opClayton@V01(V0, theta0, theta1)
## (2) build U
U1 <- opClayton@psi(rexp(500)/V0, theta0)
U2 <- opClayton@psi(rexp(500)/V01, theta1)
U3 <- opClayton@psi(rexp(500)/V01, theta1)
U <- cbind(U1, U2, U3)
@

Let us visualize the generated data with a scatter-plot matrix.
<<ex3-splom, fig=TRUE, height=5>>=
require(lattice)
print(splom(~ U, varnames = paste("U", 1:3, sep="_"), cex = 0.4))
@
...
<<ex3-explore1>>=
round(cor(U, method="kendall"), 3) 
@

The different lower and upper tail-dependence coefficients for this copula can
be obtained as follows.
<<ex3-explore2>>=
c(opClayton @ lambdaL(theta0),
  opClayton @ lambdaU(theta0))
c(opClayton @ lambdaL(theta1),
  opClayton @ lambdaU(theta1))
@
...
are implemented. As a by-product, the package implements, e.g., an efficient
