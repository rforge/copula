...
setClass("interval", contains =  "numeric", #' of length 2
	 ...
	 validity = function(object) {
	     if(length(rng <- object@.Data) != 2) "interval must be of length 2"
                 ...
                 "'range[2]' must not be smaller than range[1]"
             }
	     else TRUE
	 })
...
setClass("ACopula",
	 representation(name = "character",
                        ...
                        psiInv = "function", #' of (p, theta) -- psi_inverse: 
                                        #' \psi^{-1}(p) = t
                        theta = "numeric", #' value of theta or  'NA'  
                                        #' for unspecified
                        paraConstr = "function", #' of (theta) ; constr(theta) 
                                        #' |--> TRUE: "fulfilled"
                        ##' when theta is one-dimensional, specifying the 
                        ##' interval is more convenient:
                        paraInterval = "maybeInterval", #' [.,.]  (.,.], etc ..
                        V0 = "function",	#' of (n,theta) -- RNGenerator
                        tau= "function",	#' of (theta)
                        tauInv = "function",    #' of (tau)
                        lTDC = "function",    #' of (theta) lambda_l
                        lTDCInv = "function", #' of (lambda) Inverse of lambda_l
                        uTDC = "function",    #' of (theta) lambda_u
                        uTDCInv = "function", #' of (lambda) Inverse of lambda_u

                        ##' Nesting properties if the child copulas are of the 
                        ##' same family :
                        nestConstr = "function", #' of (th0, th1)
                                        #; TRUE <==> "fulfilled"
	 ...
	 validity = function(object) {
		 ...
		 if (length(formals(f)) < nArgs)
		     paste("slot '",sName,"' must be a function of at least ",
                           nArgs," arguments", sep="")
		 else if(chkVectorizing && nArgs <= 2) {
		     ##' test that the function can be called with NULL
		     r0 <- tryCatch(if(nArgs == 2) f(NULL, theta = th) else 
                                    f(NULL), error = function(e) e)
		     if(is(r0, "error"))
			 sprintf("calling %s(NULL, %g) fails: %s",
				 sName, th, r0$message)
		     else if(!identical(r0,
                                    {n0 <- numeric(0)
                                     if(nArgs == 2) f(n0, theta = th) else 
                                     f(n0) }))
             ...
             if(!isTRUE(tt <- checkFun("paraConstr", 1, chkVect=FALSE))) 
                 return(tt)
             if(!isTRUE(tt <- checkFun("nestConstr", 2, chkVect=FALSE))) 
                 return(tt)
...
setMethod("validTheta", signature(x = "ACopula"),
	  function(x) {
		  ...
		  if(is.finite(d <- diff(as.numeric(int)))) #' take mid-value 
		      int[1] + d/2
		  else if (all(iinf <- is.infinite(is.finite(int)))) 
                      ##' (-Inf, Inf)
		      1/2 
	      ...
	      } else  { ##' need to use parameter-contraint function and 
                  ##' "random search":
		  f.c <- x@paraConstr
		  for (th in c(1/2, (0:16)/16, round(exp(rnorm(20)),2)))
		      if(f.th <- f.c(th)) break
		  if(f.th) th else stop("could not find validTheta(<c>) for this 
		copula")
	      }
	  })
...
setClass("nACopula",
	 representation(copula = "ACopula",
                        ...
                        ##' TODO? nesting properties (V01,..) for specific mother
                        ##' -child relations
                        ),
         validity = function(object) {
             ...
             allC <- allComp(object)
             if(length(allC) != d)
                 return("must have d coordinates (from 'comps' and child 
copulas)")
             TRUE
         })
...
copulas)")
             if(!all(sort(allC) == 1:d))
                 return(paste("The implicit coordinates are not identical to 
1:d; instead\n  ",
                              paste(allC, collapse=", ")))
             TRUE
         })
