...
## specify theta_1 and theta_2 such that Kendall's tau equals the given tau
theta1 <- 0.01
theta <- c(theta1, tauInv.GIG(tau, theta1, interval=c(1e-12,100)))
...
theta1 <- c(0, 0.001, 0.5, 1, 5, 10)
cols <- colorRampPalette(c("red", "orange", "darkgreen", "turquoise", "blue"), 
                         space="Lab")(length(theta1))
for(i in seq_along(theta1))
    curve(tau.GIG.vec(theta1[i],x), 1e-12, 2, 
          main="Kendall's tau for the GIG family", ylim=c(0,1),
          xlab=expression(theta[2]), ylab=expression(tau(theta[1],theta[2])), add=(i>1), 
          lwd=1.4, col=cols[i])
label <- as.expression(lapply(1:length(theta1), function(i) substitute(theta[1]==th, list(th=theta1[i]))))
