...
Note further that the ``beautiful formula'' clearly shows the linearity of
$\rho(\th)$ for small $\th$, i.e., $\th \approx 0$.
...
So, \code{.rhoAmh.2()} using a series approximation for $|\theta|<$\code{e.sml}
...
t0 <- eval(formals(.rhoAmh.2)$e.sml)*.999999999
...
curve(Vectorize(copula:::.rhoAmhCopula)(x), n=1025, -1, 1,
      xlab= quote(theta), ylab="", ylim= c(-1,1), col="tomato", lwd=2, las=1)
...
abline(h=0,v=0, lty=3); rect(-1,-1,1,1, lty=3); x. <- .65
@
...
th <- 10^-(6:99); i <- -(1:9)
rth <- rhoAMH(th)
stopifnot(all.equal(rhoAMH(t0), t0/3 * (1 + t0/4), tol = 0.06),
