...
obtained via `rtrafo(, inverse=TRUE)`).
...
```{r, fig.align="center", fig.width=12, fig.height=6}
U.t  <- rtrafo(U,  cop=cop, inverse=TRUE) # via PRNG
U.t. <- rtrafo(U., cop=cop, inverse=TRUE) # via QRNG
par(pty="s", mfrow=1:2)
...
```{r, fig.align="center", fig.width=12, fig.height=6}
U.C  <- rtrafo(U, cop=cop, inverse=TRUE) # via PRNG
U.C. <- rtrafo(U., cop=cop, inverse=TRUE) # via QRNG
par(pty="s", mfrow=1:2)
...
U.t.3d <- rtrafo(U.3d,  cop=cop, inverse=TRUE)
par(pty="s")
pairs(U.t.3d, gap=0,
...
U <- halton(n, 3)
U_CDM <- rtrafo(U[,1:2], cop=cop, inverse=TRUE) # via CDM
                        ...
                        cop.method=c("CDM", "MO"))
{
    ...
    "CDM" = {
        ...
        lst <- apply(U., 3, FUN=function(x) list(rtrafo(x, cop=cop, inverse=TRUE)))
        lapply(lst, `[[`, 1)
    },
    "MO" = {
...
lm.p <- lm(log(sd)~log(n), data=res.p)
c.p <- exp(lm.p$coefficients[[1]])
a.p <- lm.p$coefficients[[2]]
