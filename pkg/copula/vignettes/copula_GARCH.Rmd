...
Z <- sqrt((nu.-2)/nu.) * qt(U, df = nu.) # margins (have to have mean 0 and variance 1 for ugarchpath()!)
```
...
X. <- fitted(X) # X_t = mu_t + eps_t
sig.X <- sigma(X) # sigma_t (conditional standard deviations)
eps.X <- X@path$residSim # epsilon_t = sigma_t * Z_t

## Basic checks
stopifnot(all.equal(X., X@path$seriesSim, check.attributes = FALSE)) # sanity check
stopifnot(all.equal(sig.X, X@path$sigmaSim, check.attributes = FALSE)) # sanity check
stopifnot(all.equal(eps.X, sig.X * Z, check.attributes = FALSE)) # sanity check
...
these parameters).
```{r}
## Fit marginal time series models
uspec <- ugarchspec(varModel, mean.model = meanModel, distribution.model = "std")
fit <- apply(X., 2, function(x) ugarchfit(uspec, x))
```

Check the residuals.
```{r, fig.align = "center", fig.width = 6, fig.height = 6}
## Check pseudo-observations of the residuals Z
...
```{r}
## Fit copula (and marginal t distributions) to the pseudo-observations
## of the standardized residuals
fitcop <- fitCopula(ellipCopula("t", dim = 2), data = U., method = "mpl")
nu. <- rep(nu., d) # marginal degrees of freedom; for simplicity treated as known here
...
## Extract the resulting series and plot (X_t) for each margin
