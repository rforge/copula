...
rhoDerTrue <- sapply(alphaGrid, function(x) rhoDerFrankCopula(frankCopula(x)))
...
curve(frankRhoDer(atanh(x) * 40), col="blue", xlab=expression(alpha), ylab=bquote(rho~"'"))
lines(thetaGrid, rhoDerTrue)
legend("topleft", legend=c("true", "numerical"), col=c("black", "blue"), lty=c(1,2), cex=0.7)
dev.off()
...
plot(thetaGrid, frankRhoDer(atanh(thetaGrid) * 40) - rhoDerTrue, type="l", xlab=expression(alpha), ylab=bquote(rho~"'"))
dev.off()

load("t4Rho.rda")
rhoTrue <- sapply(thetaGrid, function(x) spearmansRho(tCopula(x)))
rhoDerTrue <-  6 / (pi * sqrt(4 - thetaGrid^2)) 
...
curve(t4RhoDer(x), col="blue", xlab=expression(theta), ylab=bquote(rho~"'"), lty = 2)
lines(thetaGrid, rhoDerTrue)
legend("bottomright", legend=c("normal", "numerical"), col=c("black", "blue"), lty=c(1,2), cex=0.7)
dev.off()
...
plot(thetaGrid, t4RhoDer(thetaGrid) - rhoDerTrue, type="l", xlab=expression(theta), ylab=bquote("difference in "~rho~"'"))
dev.off()
