...
These and the derivative (\ref{eq:psiInvD1})
originally were
<<def-orig-psi-func>>=
...
@
and the general $k$-th derivative (\ref{eq:psiDabs}), simplified, for $k= 1$
(\code{degree = 1}),
<<def-orig-psiDabs>>=
...
$\psiis{\theta}$, i.e, (\ref{eq:psiInvFrank}),
...
simplistic \code{psiInv.0()} function instead of the more stable
\code{psiInv.2()} one:
<<check-psiInv>>=
